...
static bool check_lldi_args(int *arguments, char *coding_byte, robot_t *robot)
{
    for (int i = 0; i < 2; i++) {
        ...
        if (coding_byte[i] == T_IND) {
            arguments[i] = robot->read_index + arguments[i];
        }
    }
...
static int *parse_lldi_args(memory_t *memory, robot_t *robot,
    char *coding_byte)
{
...
static int second_read_lldi(corewar_t *corewar, robot_t *robot,
    int previous_result)
{
    int index = get_address(robot->read_index + previous_result - 1);
    int result = 0;
...
void lldi_instruction(corewar_t *corewar, robot_t *robot)
{
    ...
    int *args = parse_lldi_args(corewar->memory, robot, c_byte);
    int result = 0;
    ...
    if (check_lldi_args(args, c_byte, robot) == false) {
