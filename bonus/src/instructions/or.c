...
static int *read_or_args(memory_t *memory, robot_t *robot,
    char *coding_byte)
{
    ...
    if (!arguments) {
        my_puterr("read_or_args: Memory allocation failed\n");
        free(coding_byte);
        return NULL;
    }
    for (int i = 0; i < 2; i++) {
...
static bool check_or_args_are_valid(int *args, char *c_byte, robot_t *robot)
{
    for (int i = 0; i < 2; i++) {
...
void or_instruction(corewar_t *corewar, robot_t *robot)
{
    ...
    int *args = read_or_args(corewar->memory, robot, c_byte);
    int result;

    if (!args)
        return;
    robot->carry = 0;
    if (check_or_args_are_valid(args, c_byte, robot) == false) {
